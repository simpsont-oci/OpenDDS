...
RtpsUdpDataLink::RtpsUdpDataLink(RtpsUdpTransport& transport,
  ...
  , heartbeat_reply_(reactor_task->interceptor(), config.heartbeat_response_delay_, *this, &RtpsUdpDataLink::send_heartbeat_replies)
                                     ...
                                     OpenDDS::DCPS::DiscoveryListener* listener)
{
  ...
  g.release();
  if (enableheartbeat) {
    heartbeat_.schedule(heartbeat_period());
  }
}
  ...
  if (wi != remote_writers_.end()) {
    ...
    if (writer->recvd_.empty()) {
      ...
      if (Transport_debug_level > 5) {
        ...
        ACE_DEBUG((LM_DEBUG, ACE_TEXT("%T (%P|%t) RtpsUdpDataLink::process_data_i(DataSubmessage) -")
    ...
    if (Transport_debug_level > 5) {
      ...
      GuidConverter rc(id_);
      ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) RtpsUdpDataLink::process_data_i(DataSubmessage) -")
                           ACE_TEXT(" data seq: %q from %C to %C dropped because of unknown reader\n"),
...
RtpsUdpDataLink::RtpsReader::gather_preassociation_ack_nacks_i(MetaSubmessageVec& meta_submessages)
{
  ...
  for (WriterInfoSet::const_iterator pos = preassociation_writers_.begin(), limit = preassociation_writers_.end();
       pos != limit; ++pos) {
    gather_preassociation_ack_nack_i(meta_submessages, *pos);
  }
}

void
RtpsUdpDataLink::RtpsReader::gather_preassociation_ack_nack_i(MetaSubmessageVec& meta_submessages,
...
RtpsUdpDataLink::bundle_and_send_submessages(MetaSubmessageVecVec& meta_submessages)
{
  ...
  size_t total_size = 0;
  for (MetaSubmessageVecVec::const_iterator it = meta_submessages.begin(); it != meta_submessages.end(); ++it) {
    total_size += it->size();
  }
...
RtpsUdpDataLink::send_heartbeat_replies(const DCPS::MonotonicTimePoint& /*now*/)
{
...

  {
    ...
    for (InterestingAckNackSetType::const_iterator pos = interesting_ack_nacks_.begin(),
      ...
      const RtpsReaderMap::iterator rr = readers_.find(pos->readerid);
      const int count = rr == readers_.end() ? ++acknack_counts_[pos->readerid.entityId] : rr->second->inc_acknack_count();
      AckNackSubmessage acknack = {
        {ACKNACK,
         CORBA::Octet(FLAG_E | FLAG_F),
         0 /*length*/},
...
void
RtpsUdpDataLink::RtpsWriter::gather_preassociation_heartbeat_i(MetaSubmessageVec& meta_submessages,
                                                               MetaSubmessage& meta_submessage,
                                                               const ReaderInfo_rch& reader)
{
